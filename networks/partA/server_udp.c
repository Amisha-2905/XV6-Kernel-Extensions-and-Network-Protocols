...
void send_board(int socketfd, struct sockaddr_in *client_addr, socklen_t client_len)
{
    ...
    if (sendto(socketfd, board_status, strlen(board_status), 0, (struct sockaddr *)client_addr, client_len) < 0)
    {
...
void receive_board(int socketfd, struct sockaddr_in *client_addr, socklen_t *client_len)
{
    ...
    if (recvfrom(socketfd, board_status, sizeof(board_status), 0, (struct sockaddr *)client_addr, client_len) < 0)
    {
...
int main(int argc, char *argv[])
{
    ...
    if (recvfrom(socketfd, buffer, sizeof(buffer), 0, (struct sockaddr *)&client1_addr, &client1_len) < 0)
    {
    ...
    if (sendto(socketfd, buffer, strlen(buffer), 0, (struct sockaddr *)&client1_addr, client1_len) < 0)
    {
    ...
    memset(buffer, 0, sizeof(buffer));
    if (recvfrom(socketfd, buffer, sizeof(buffer), 0, (struct sockaddr *)&client2_addr, &client2_len) < 0)
    {
        perror("Name not able to receive!");
    }
    ...
    if (sendto(socketfd, buffer, strlen(buffer), 0, (struct sockaddr *)&client2_addr, client2_len) < 0)
    {
    ...
    while (1)
    {
        ...
        while (winner == -1 && if_board_full() != 0)
        {
            ...
            if (turn % 2 == 0)
            {
                send_board(socketfd, &client2_addr, client2_len);
                receive_board(socketfd, &client2_addr, &client2_len);
            }
            else
            {
                send_board(socketfd, &client1_addr, client1_len);
                receive_board(socketfd, &client1_addr, &client1_len);
            }
            winner = check_winner();
            turn++;
        }
