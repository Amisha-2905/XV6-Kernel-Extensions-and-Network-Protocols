...
void send_board(int socketfd, struct sockaddr_in *server_addr, socklen_t addr_len)
{
    ...
    if (sendto(socketfd, board_status, strlen(board_status), 0, (struct sockaddr *)server_addr, addr_len) < 0)
    {
...
void receive_board(int socketfd, struct sockaddr_in *server_addr, socklen_t *addr_len)
{
    ...
    if (recvfrom(socketfd, board_status, sizeof(board_status), 0, (struct sockaddr *)server_addr, addr_len) < 0)
    {
...
int main(int argc, char *argv[])
{
    ...
    if (sendto(socketfd, buffer, strlen(buffer), 0, (struct sockaddr *)&server_addr, addr_len) < 0)
    {
        perror("Sending Name");
    }
